...
APIs) instead of the fakes in `src/lunar_tools_art/tools/headless.py`. Current baseline: 176
passed, 14 xfailed, 0 failed, 8 warnings.
...
- **Emotion Detection (`src/lunar_tools_art/emotion.py`)**: Face detection via OpenCV Haar cascade, with a FER+ ONNX emotion classifier fetched via `scripts/fetch_models.py` (on-machine verification against real hardware still pending). Fallback chain: FER+ ONNX -> OpenCV DNN ONNX -> Haar cascade placeholder. `has_classifier` property indicates when a real model is loaded.
...
- **Test suite**: `LUNAR_HEADLESS=1 pytest -q` → 176 passed, 14 xfailed, 0 failed, 8 warnings.
