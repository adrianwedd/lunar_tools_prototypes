...
class SpeechArtGenerator:
    def __init__(
        self, lunar_tools_art_manager: Manager, no_speech_delay=1, loop_delay=5
    ):
    ...
    def run(self):
        self.lunar_tools_art_manager.logger.info(
            "Speech Activated Art: Press 'q' to quit."
        )
        while True:
            if self.keyboard_input.is_key_pressed("q"):
                self.lunar_tools_art_manager.logger.info(
                    "Exiting Speech Activated Art."
                )
                break
            ...
            try:
                ...
                self.lunar_tools_art_manager.logger.info(
                    "Generating prompt from speech..."
                )
                prompt = self.gpt4.generate(
                    f"Create a visual representation of: {speech_text}"
                )
                self.lunar_tools_art_manager.logger.info(f"Generated prompt: {prompt}")

                if not prompt:
                    self.lunar_tools_art_manager.logger.info(
                        "Failed to generate prompt, trying again."
                    )
                    time.sleep(self.no_speech_delay)
                    continue
