...
class ChatRoomNarrativeQuilt:
    def __init__(
        ...
        self.quilt_image = Image.new(
            "RGB", (self.renderer.width, self.renderer.height), color=(200, 200, 200)
        )  # Light gray canvas
    ...
    def _generate_panel_graphic(self, description):
        ...
        try:
            ...
            self.logger.error(
                f"Error generating panel graphic for '{description}': {e}",
                exc_info=True,
            )
            return Image.new(
                "RGB", (self.patch_size[0], self.patch_size[1]), color=(100, 100, 100)
            )  # Placeholder
    ...
    def _composite_patch(self, panel_graphic, message):
        ...
        wrapper = textwrap.TextWrapper(
            width=int(self.patch_size[0] / (self.font_size * 0.6))
    ...
    def run(self):
        ...
        if self.flask_app and self.socketio:
            self.logger.info(
                "Flask server and Socket.IO enabled. Access via web browser."
            )
            ...
            self.logger.info(
                "Flask and Socket.IO not configured. Using keyboard input for messages."
            )
