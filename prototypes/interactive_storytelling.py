...
class InteractiveStoryteller:
    def __init__(self, lunar_tools_art_manager: Manager, recording_duration=5, summary_interval=10):
        ...
        self.recording_duration = recording_duration # Default recording duration in seconds
    ...
    def _summarize_story_history(self):
        if len(self.story_history) > self.summary_interval:
            ...
            try:
                ...
                self.lunar_tools_art_manager.logger.error(f"Error during story summarization: {e}", exc_info=True)
                self.lunar_tools_art_manager.logger.info("Failed to summarize story history. Continuing without summarization for this turn.")
            ...
            current_story_tokens = len(full_history_to_summarize.split()) # Simplified token count (word count)

            if current_story_tokens > MAX_TOKENS:
                self.lunar_tools_art_manager.logger.info("Story history exceeding token limit. Summarizing...")
                summary_prompt = f"Summarize the following story history concisely, focusing on key plot points, character arcs, and unresolved conflicts, in a way that can be used to continue the narrative:\n\n{full_history_to_summarize}\n\nSummary:"
