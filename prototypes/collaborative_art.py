...
class CollaborativeArtServer:
    def __init__(self, lunar_tools_art_manager: Manager, ip='127.0.0.1', port='5556'):
        ...
        self.stable_diffusion = self.lunar_tools_art_manager.sdxl_turbo # Assuming SDXL_TURBO can be used for image generation
    ...
    def run(self):
        self.lunar_tools_art_manager.logger.info("Collaborative Art Server: Press 'q' to quit.")
        client_thread = threading.Thread(target=self.handle_client)
        client_thread.start()
        while self.running:
            if self.keyboard_input.is_key_pressed('q'):
                self.lunar_tools_art_manager.logger.info("Exiting Collaborative Art Server.")
                self.running = False
                break
