...
from collections import deque # Import deque here for Particle class
...
class Particle:
    ...
    def draw(self, draw):
        # Draw particle
        draw.ellipse([self.x - self.size, self.y - self.size, self.x + self.size, self.y + self.size], fill=self.color)
        # Draw trail
        for i, (tx, ty) in enumerate(self.trail):
            alpha = int(255 * (i / self.trail.maxlen))
            trail_color = self.color[:3] + (alpha,)
            draw.ellipse([tx - self.size/2, ty - self.size/2, tx + self.size/2, ty + self.size/2], fill=trail_color)

class VirtualCloudChamber:
    def __init__(self, lunar_tools_art_manager: LunarToolsArtManager, loop_delay=0.05):
    ...
    def _generate_vapor_background(self):
        ...
        img = Image.new('RGBA', (self.renderer.width, self.renderer.height), color = (50, 50, 70, 255)) # Ensure background is RGBA
        draw = ImageDraw.Draw(img)
        for _ in range(100):
            x = random.randint(0, self.renderer.width)
            y = random.randint(0, self.renderer.height)
            radius = random.randint(50, 200)
            color = (random.randint(80, 120), random.randint(80, 120), random.randint(100, 150), random.randint(20, 80)) # Semi-transparent blueish
    ...
    def _create_new_particle(self):
        ...
        color = (random.randint(150, 255), random.randint(150, 255), random.randint(150, 255), 255) # Whiteish, opaque
    ...
    def _narrate_particle_journeys(self):
        ...
        interesting_particles = random.sample(self.particles, min(len(self.particles), 3))
        descriptions = []
        for p in interesting_particles:
            descriptions.append(f"A particle starting at ({int(p.trail[0][0])}, {int(p.trail[0][1])}) drifting towards ({int(p.x)}, {int(p.y)}) with a {len(p.trail)}-step trail.")
        
        prompt = f"Narrate the journeys of these particles in a poetic and scientific tone:\n\n" + "\n".join(descriptions)
        ...
        try:
            ...
            self.text2speech.generate(narration, filename=".output/narration.mp3") # Generate speech to a file
            self.lunar_tools_art_manager.sound_player.play_sound(".output/narration.mp3") # Play the generated speech
        except Exception as e:
            self.logger.error(f"Error generating or playing narration: {e}", exc_info=True)

    def run(self):
        self.logger.info("Virtual Cloud Chamber: Press 'q' to quit. Speak or press any key to create particles.")
        ...
        while True:
            ...
            try:
                speech_text = self.speech2text.transcribe(duration=1) # Short recording for quick detection
                if speech_text and len(speech_text.split()) > 1: # Basic check for meaningful speech
            ...
            for particle in list(self.particles): # Iterate over a copy to allow removal
                particle.update()
                particle.draw(draw)
                # Remove particles that go off-screen
                if not (0 < particle.x < self.renderer.width and 0 < particle.y < self.renderer.height):
