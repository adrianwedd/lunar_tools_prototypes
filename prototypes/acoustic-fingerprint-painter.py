...
class AcousticFingerprintPainter:
    def __init__(self, lunar_tools_art_manager: Manager):
        ...
        self.canvas_image = Image.new('RGB', (self.renderer.width, self.renderer.height), color = (0, 0, 0)) # Black canvas

    def _record_audio_sample(self, duration=3):
        self.logger.info(f"Recording audio sample for {duration} seconds...")
        file_path = "temp_audio_sample.wav"
    ...
    def _extract_audio_features(self, audio_file_path):
        ...
        try:
            ...
            self.logger.error(f"Error during audio feature extraction: {e}", exc_info=True)
            return [0.0] * (13 * 2 + 2) # Return a default vector on error (13 MFCCs * 2 (mean/std) + 2 (centroid mean/std))

    def _get_stroke_parameters_from_gpt4(self, feature_vector):
        prompt = f"Given the following audio feature vector: {feature_vector}. " \
        ...
        try:
            ...
            self.logger.error(f"Error decoding JSON from GPT-4 response: {e}", exc_info=True)
            self.logger.error(f"GPT-4 response: {response}")
            return None
        except Exception as e:
            self.logger.error(f"Error getting stroke parameters from GPT-4: {e}", exc_info=True)
            return None
    ...
    def _draw_stroke(self, stroke_params):
        ...
        for i in range(num_segments):
            ...
            line_color = (color[0] + random.randint(-20, 20), 
            ...
            draw.ellipse([next_x - line_width/2, next_y - line_width/2, next_x + line_width/2, next_y + line_width/2], fill=line_color)

            current_x, current_y = next_x, next_y

    def run(self):
        self.logger.info("Acoustic Fingerprint Painter: Press 'q' to quit. Press 'r' to record audio and paint.")
        while True:
