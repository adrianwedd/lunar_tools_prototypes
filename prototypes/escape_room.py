...
class EscapeRoomGame:
    def __init__(self, lunar_tools_art_manager: Manager, correct_answer_sound="correct_answer.mp3", hint_sound="hint.mp3"):
        ...
        self.puzzles = [
            {"description": "a locked wooden chest with strange symbols", "solution": "open chest"},
            {"description": "a dimly lit corridor with a flickering light", "solution": "go forward"},
            {"description": "a complex ancient mechanism with missing gears", "solution": "fix mechanism"}
        ]

    def generate_next_puzzle_visual(self):
        if self.game_state["current_puzzle"] < len(self.puzzles):
            puzzle_description = self.puzzles[self.game_state["current_puzzle"]]["description"]
            ...
            try:
                ...
                self.lunar_tools_art_manager.logger.error(f"Error generating puzzle visual for '{puzzle_description}': {e}", exc_info=True)
            ...
            try:
                ...
                self.lunar_tools_art_manager.logger.error(f"Error generating game over visual: {e}", exc_info=True)
                return None
    ...
    def run(self):
        ...
        if self.game_state["current_puzzle"] < len(self.puzzles):
            ...
            self.logger.info(f"Current puzzle: {self.puzzles[self.game_state['current_puzzle']]['description']}")
        else:
        ...
        while True:
            ...
            if command:
                ...
                intent = self.lunar_tools_art_manager.gpt4.generate(intent_prompt).strip().lower()
                self.logger.info(f"Detected intent: {intent}")

                if intent == "solve_puzzle":
                    current_puzzle_solution = self.puzzles[self.game_state["current_puzzle"]]["solution"]
                    if current_puzzle_solution.lower() in command.lower():
                        ...
                        self.game_state["puzzles_solved"].append(self.puzzles[self.game_state["current_puzzle"]])
                        self.game_state["current_puzzle"] += 1

                        if self.game_state["current_puzzle"] < len(self.puzzles):
                            self.logger.info(f"Moving to next puzzle: {self.puzzles[self.game_state['current_puzzle']]['description']}")
                            image = self.generate_next_puzzle_visual()
                            self.renderer.render(image)
                        else:
                            self.logger.info("All puzzles solved! Game Over!")
                            self.game_state["game_over"] = True
                            image = self.generate_next_puzzle_visual() # Generate game over visual
                            self.renderer.render(image)
