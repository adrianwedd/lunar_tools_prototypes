...
class TimeShiftedEchoChamber:
    def __init__(self, lunar_tools_art_manager: Manager, clip_duration=5, buffer_size=3, loop_delay=0.1):
    ...
    def _process_and_play_clip(self, clip_path):
        self.logger.info(f"Processing and playing {clip_path} with delay {self.delay_factor:.1f} and pitch {self.pitch_factor:.1f}...")
        try:
            ...
            if self.delay_factor != 1.0:
                ...
                audio_segment = audio_segment.speedup(playback_speed=1.0/self.delay_factor, chunk_size=150, crossfade=25)
            
            # Apply pitch shift
            if self.pitch_factor != 1.0:
                audio_segment = audio_segment.set_frame_rate(int(audio_segment.frame_rate * self.pitch_factor))
            
            play(audio_segment) # Play with pydub
        except Exception as e:
            self.logger.error(f"Error processing/playing audio clip {clip_path}: {e}", exc_info=True)

    def run(self):
        self.logger.info("Time-Shifted Echo Chamber: Press 'q' to quit.")
        self.logger.info("Use 'd' to increase delay, 'D' to decrease. 'p' to increase pitch, 'P' to decrease.")
        ...
        while True:
            ...
            for i, clip in enumerate(list(self.audio_buffer)):
                ...
                time.sleep(self.clip_duration * self.delay_factor * (i + 1) / self.audio_buffer.maxlen)
