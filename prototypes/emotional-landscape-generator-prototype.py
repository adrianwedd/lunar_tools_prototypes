...
class EmotionDetector:
    ...
    def detect_emotion(self):
        try:
            ...
            visitor_input = self.speech2text.transcribe(
                duration=5
            ...
            if visitor_input:
                ...
                if emotion in ["happy", "sad", "angry", "calm", "excited"]:
                    ...
                    self.logger.warning(
                        f"GPT-4 returned an unrecognised emotion: {emotion}. Defaulting to 'calm'."
                    )
                    return "calm"
            else:
                self.logger.info(
                    "No speech detected for emotion analysis. Defaulting to 'calm'."
                )
                return "calm"
    ...
    def generate_landscape(self, emotion):
        ...
        try:
            ...
            self.lunar_tools_art_manager.logger.error(
                f"Error generating image for emotion {emotion}: {e}", exc_info=True
            )
            return None

    def run(self):
        self.lunar_tools_art_manager.logger.info(
            "Emotional Landscape Generator: Press 'q' to quit."
        )
        while True:
            if self.keyboard_input.is_key_pressed("q"):
                self.lunar_tools_art_manager.logger.info(
                    "Exiting Emotional Landscape Generator."
                )
                break
