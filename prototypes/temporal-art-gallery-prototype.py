...
class TemporalArtGallery:
    def __init__(self, lunar_tools_art_manager: Manager, prompts=None, check_interval=60):
    ...
    def generate_artwork(self, prompt):
        try:
            image, _ = self.flux.generate(prompt, image_size="square_hd", num_inference_steps=4, seed=int(time.time()))
            return image
        except Exception as e:
            self.logger.error(f"Error generating artwork for prompt '{prompt}': {e}", exc_info=True)
            return None
        ...
        while True:
            ...
            if current_index != last_index:
                ...
                if image is not None:
                    ...
                    self.logger.info("Skipping rendering due to image generation error.")
